    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            try {
                ...
                // 3. 动态风速调节 (分程控制 + 温度补偿)
                // 基础风速：由CFC驱动
                let fanSpeed_actual = 1 + cfc * 1.19;

                // 温度补偿：如果送风温度太低 (saTemp < saSet)，增加风速以提高出风温度
                // 增益：每低1度，增加 15% 风速 (原5%)，大幅提升响应速度
                if (saError < 0) {
                    const lowTempBoost = Math.abs(saError) * 15;
                    fanSpeed_actual += lowTempBoost;
                }

                fanSpeed_actual = Math.max(1, Math.min(120, fanSpeed_actual));
